...
public class BoardDBBean {
	...
	//.jsp페이지에서 DB연동빈인 BoardDBBean 클랫의 메소드에 접근시 필요
	...
	public void insertArticle(BoardDataBean article)throws Exception{
		...
		try{//글의 그룹화 아이디를 결정하기 위한 부분이다.
			conn=getConnetion();
			pstmt=conn.prepareStatement("select max(num) from board");//현재 레코드에 저장되어 있는 글번호 중 가장 큰값을 검색함
			rs=pstmt.executeQuery();
			
			if(rs.next())
				number=rs.getInt(1)+1;//board테이블에 글이 입력되어 있으면 입력된 글 중 가장 큰 값 +1을 number값으로 저장
			
			else
				number=1;//없으면 number를 1로 저장
			if(num!=0){//제목글과 답변글의 순서를 결정하는 작업을 수행한다.
...
;			System.out.println(sql);
	...
	//board테이블에 저장된 전체글의 수 마지막 글번호를 얻어냄(select문)<=list.jsp에서 사용
	...
	//글의 목록(복수개의 글)을 가져옴(select문) <=list.jsp 사용
	public List<BoardDataBean> getArticles(int start,int end){
	 ...
	 //글의 내용일 보기(1개의 글)(select문)<=content.jsp 페이지에서 사용
